...
class GRUPredict(nn.Module):
    ...
    def init_hidden(self):
        ...
        if self.use_cuda:
            hx = hx.cuda()
        hidden = autograd.Variable(hx)  # convert to Variable as late as possible
        return hidden
...
def try_hyper_para(hidden_size_list, num_layer_list, data_loader, epoc, count_max):
    for hidden_size in hidden_size_list:
        for num_layers in num_layer_list:
            model = GRUPredict(input_size=TAG_SIZE, hidden_size=hidden_size, num_layers=num_layers, tag_size=TAG_SIZE)
            train_model(model, learning_rate=0.001, data_loader=data_loader, epoch=epoc, count_max=count_max)
            print("finished training!")
            model_name = 'adam-gru-' + str(hidden_size) + '-' + str(num_layers) + '.model'

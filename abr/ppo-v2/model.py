...
class Model(nn.Module):
    ...
    def forward(self, inputs, batch_size=1):
        ...
        split_2 = F.relu(self.a_conv2(inputs[:, 2:3, :5])).view(batch_size, -1)
        split_3 = F.relu(self.a_conv3(inputs[:, 3:4, :5])).view(batch_size, -1)
        ...
        split_2 = F.relu(self.c_conv2(inputs[:, 2:3, :5])).view(batch_size, -1)
        split_3 = F.relu(self.c_conv3(inputs[:, 3:4, :5])).view(batch_size, -1)

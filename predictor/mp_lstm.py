...
def try_hyper_para(hidden_size_list, num_layer_list, data_loader, epoch, count_max):
    for hidden_size in hidden_size_list:
        for num_layers in num_layer_list:
            model = LSTMPredict(input_size=4, hidden_size=hidden_size, num_layers=num_layers, tag_size=4)
            main_train(data_loader, hidden_size=hidden_size, num_layers=num_layers, num_processes=45, epoch=epoch,
                       count_max=count_max)
...
if __name__ == "__main__":
    ...
    num_layer_list = [1]
    try_hyper_para(hidden_size_list, num_layer_list, data_loader, epoch=2, count_max=200000)
